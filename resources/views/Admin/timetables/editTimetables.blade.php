...
@section("topSection")
    <div class="col-12 col-xl-8 mb-4 mb-xl-0">
        <h3 class="font-weight-bold">Edit Timetable Entry</h3>
    </div>
@endsection
...
@section('content')
    <div class="col-12 mt-3">
        <div class="row">
            <form style="width: 100%" action="{{ route('admin.timetables.update', $timetable->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="row">
                        ...
                        <select class="form-control" name="instructor_course_assignment_id" id="instructor_course_assignment_id">
                            ...
                            @foreach($assignments as $assignment)
                                <option
                                    ...
                                    {{ $timetable->instructor_course_assignment_id == $assignment->id ? 'selected' : '' }}
                                >
                    ...
                    <div class="col-lg-6 mt-3">
                        ...
                        <input class="form-control" value="{{ old('start_time', $timetable->start_time) }}" type="time" name="start_time" id="start_time">
                        @error('start_time')
                            <p class="text-danger">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="col-lg-6 mt-3">
                        ...
                        <input class="form-control" value="{{ old('end_time', $timetable->end_time) }}" type="time" name="end_time" id="end_time">
                        @error('end_time')
                            <p class="text-danger">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="col-lg-6 mt-3">
                        ...
                        <input class="form-control" value="{{ old('room', $timetable->room) }}" type="text" name="room" id="room">
                        @error('room')
                            <p class="text-danger">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="col-lg-12 mt-3">
                        ...
                        <textarea class="form-control" name="notes" id="notes" rows="3">{{ old('notes', $timetable->notes) }}</textarea>
                        @error('notes')
                            <p class="text-danger">{{ $message }}</p>
                        @enderror
                    </div>
    ...
    <script>
        function gatherTimetableData() {
            ...
            return {
                ...
                room: $('#room').val(),
                timetable_id: {{ $timetable->id }} // Include the current timetable ID for update
            };
        }
        ...
        function checkConflict() {
            ...
            formData.append('timetable_id', {{ $timetable->id }}); // Include timetable ID for update
